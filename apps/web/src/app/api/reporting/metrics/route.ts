...
import { getBusinessMetrics } from '@blade/core'
...
export async function GET(req: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const data = getBusinessMetrics(Number.isNaN(period) ? 7 : period)
    return Response.json({ success: true, data }, { headers: corsHeaders() })
  } catch (error: unknown) {
    const msg = error instanceof Error ? error.message : 'Failed to get metrics'
    logger.error('Reporting', `GET /metrics error: ${msg}`)
