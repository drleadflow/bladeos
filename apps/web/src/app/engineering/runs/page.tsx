...
const EVENT_TYPE_STYLES: Record<
  string,
  { label: string; tone: 'blue' | 'cyan' | 'emerald' | 'rose' | 'amber' | 'neutral' }
> = {
...
function getEventHref(event: ActivityEvent): string | null {
  if (event.targetType === 'worker' && event.targetId) return '/engineering/workers'
  if (event.jobId) return '/engineering/jobs'
  if (event.conversationId) return '/'
  if (event.eventType === 'approval') return '/today'
  return null
}
...
export default function EngineeringRunsPage() {
  ...
  const workerActionCount = events.filter((e) => e.eventType.startsWith('worker_')).length
  const totalCost = events.reduce((sum, e) => sum + e.costUsd, 0)

  return (
    <PageShell
      eyebrow="Engineering / Runs"
      title="Live execution timeline"
      description="The operational feed of what Blade is doing right now — what has completed and where intervention may be needed."
      actions={
    ...
    >
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <MetricCard
          ...
          hint={`${approvalCount} approvals, ${workerActionCount} worker actions`}
          accent="amber"
        />
      </div>
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-[0.82fr_1.18fr]">
        <Panel>
          <PanelHeader
            ...
            description="Focus on the type of work you want to inspect."
          />
              ...
              <button
                ...
                className={`rounded-full px-3 py-2 text-xs font-medium uppercase tracking-[0.18em] transition-all ${
                  filter === f.value
                    ...
                    : 'border border-white/10 bg-white/[0.04] text-zinc-400 hover:border-blue-400/20 hover:bg-white/[0.08] hover:text-zinc-200'
                }`}
              >
                {f.label}
              </button>
        ...
        <Panel glow="cyan">
          ...
          {loading ? (
            ...
            <div ref={feedRef} className="max-h-[calc(100vh-300px)] space-y-3 overflow-y-auto pr-1">
              {events.map((event) => {
                    ...
                    <div className="flex items-start gap-4">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-3 flex flex-wrap items-center gap-4 text-xs text-zinc-500">
                          ...
                          {event.conversationId && <span>Conversation {event.conversationId.slice(0, 8)}</span>}
                          {event.jobId && <span>Job {event.jobId}</span>}
                          {event.costUsd > 0 && <span className="text-amber-300">{formatCost(event.costUsd)}</span>}
                          {href && (
                            <Link href={href} className="text-blue-300 transition-colors hover:text-blue-200">
